...
class GUIClass():
    def __init__(self):
        ...
        self.AutoDockingB = Button(self.root, text = "Auto Docking Live", command = lambda: autodockingloop(self.cap, self.nav_in_queue))
        ...
        StartNavB = Button(self.root, text = "Start Nav Process", command = lambda: navProcessStart.startNavProcess(self.nav_in_queue, self.nav_out_queue))
        StartNavB.grid(row = 0, column = self.vcol + 2, sticky = 'n')

        EndNavB = Button(self.root, text = "Terminate Nav Process", command = navProcessStart.terminateNavProcess)
        EndNavB.grid(row = 1, column = self.vcol + 2, sticky = 'n')
